...
Evidence: `db/002_eval_retrieval_workflow.sql` / line=`38` / object=`eval.answer_runs`  
Evidence: `db/002_eval_retrieval_workflow.sql` / line=`350` / object=`rag.match_public_chunks`
...
Evidence: `db/004_materialized_marts.sql` / line=`72` / object=`mart.mv_sales_order`, grain=`1 row per txn_id`  
...
Evidence: `db/optimization_round2.md` / line=`22` / object=`rag.hybrid_search_public_chunks`  
Evidence: `db/optimization_round2.md` / line=`32` / object=`CREATE OR REPLACE FUNCTION rag.hybrid_search_public_chunks`
...
Evidence: `db/optimization_round2.md` / line=`18` / object=`db/006_hybrid_retrieval.sql`  
Evidence: `db/optimization_round2.md` / line=`32` / object=`rag.hybrid_search_public_chunks`, columns=`query_embedding`, `query_text`, `match_count`
...
ควรรัน full pipeline จริงบน local Postgres แล้วเก็บผลทุกคำตอบลง `eval.answer_runs` เพื่อให้ตรวจย้อนกลับได้ว่าแต่ละ answer ใช้ SQL/source อะไร  
Evidence: `db/002_eval_retrieval_workflow.sql` / line=`38` / object=`eval.answer_runs`  
Evidence: `questions.csv` / rows=`100` / columns=`id`, `question`
...
Evidence: `db/004_materialized_marts.sql` / line=`72` / object=`mart.mv_sales_order`  
Evidence: `db/005_rag_hnsw_and_public_chunks_mv.sql` / line=`17` / object=`rag.mv_public_retrievable_chunks`  
Evidence: `db/optimization_round2.md` / line=`18` / object=`db/006_hybrid_retrieval.sql`

8. ถามว่า “จะรู้ได้ยังไงว่าคำตอบแต่ละข้ออ้างอิงถูก?”  
คำตอบที่ควรเตรียม: ทุก answer run ต้องเก็บ SQL/source/confidence/runtime ลง `eval.answer_runs` และเทียบ row count/grain ของ mart views กับ base tables  
Evidence: `db/002_eval_retrieval_workflow.sql` / line=`38` / object=`eval.answer_runs`  
Evidence: `fahmai_model_database_schema.md` / line=`80` / object=`mart.v_sales_order`, grain=`1 row per fact_sales.txn_id`  
Evidence: `fahmai_model_database_schema.md` / line=`81` / object=`mart.v_sales_line`, grain=`1 row per fact_sales_line_item.line_item_id`
...
| Eval question table | `db/002_eval_retrieval_workflow.sql` | line=`15` | `eval.questions` |
| Eval answer trace table | `db/002_eval_retrieval_workflow.sql` | line=`38` | `eval.answer_runs` |
