...
Key table sizes:
- `core.fact_loyalty_ledger` — 118,857 rows in the public CSV checked in this workspace. Treat earlier 11.6M estimates as non-public/private-dataset assumptions until verified.
- `core.fact_inventory_movement` — 310,827 rows
- `core.fact_sales_line_item` — 309,129 rows
- `core.fact_sales` — 117,105 rows
- `core.fact_bank_transaction` — 65,334 rows
...
### 1.2 `core.fact_inventory_movement`
...
### 1.3 `core.fact_inventory_monthly_snapshot`
...
CREATE INDEX IF NOT EXISTS fact_warranty_claim_sku_idx
    ON core.fact_warranty_claim (sku_id);
```

### 1.5 `core.fact_return`
...
CREATE INDEX IF NOT EXISTS fact_return_sku_idx
    ON core.fact_return (sku_id);
```

### 1.6 `core.fact_cs_interaction`
...
CREATE INDEX IF NOT EXISTS fact_cs_interaction_channel_type_idx
    ON core.fact_cs_interaction (channel, interaction_type);
```

### 1.7 `core.dim_customer` (analytics indexes)
...
CREATE INDEX IF NOT EXISTS dim_customer_type_region_idx
    ON core.dim_customer (customer_type, region);
```
...
CREATE INDEX IF NOT EXISTS fact_sales_paid_date_branch_idx
    ON core.fact_sales (business_event_date, branch_code)
    WHERE payment_status = 'paid';

CREATE INDEX IF NOT EXISTS fact_sales_b2b_customer_idx
    ON core.fact_sales (customer_id, business_event_date)
    WHERE is_b2b = true;

CREATE INDEX IF NOT EXISTS dim_employee_active_branch_dept_idx
    ON core.dim_employee (branch_code, dept_code)
    WHERE status = 'active';
```
...
Convert heavy `mart.*` views to materialized views and create a refresh function.

### 2.1 Drop existing regular views first

```sql
DROP VIEW IF EXISTS mart.v_bank_reconciliation;
DROP VIEW IF EXISTS mart.v_sales_deposit_batch_reconciliation;
DROP VIEW IF EXISTS mart.v_sales_order;
DROP VIEW IF EXISTS mart.v_sales_line;
DROP VIEW IF EXISTS mart.v_vendor_payment;
```

### 2.2 `mart.mv_sales_deposit_batch_reconciliation`

Must be created before `mv_bank_reconciliation` (dependency order).

```sql
CREATE MATERIALIZED VIEW mart.mv_sales_deposit_batch_reconciliation AS
...
COMMENT ON MATERIALIZED VIEW mart.mv_sales_deposit_batch_reconciliation IS
...
CREATE INDEX mv_sales_deposit_batch_date_branch_idx
    ON mart.mv_sales_deposit_batch_reconciliation (business_event_date, branch_code);

CREATE INDEX mv_sales_deposit_batch_id_idx
    ON mart.mv_sales_deposit_batch_reconciliation (sales_deposit_batch_id);
```

### 2.3 `mart.mv_sales_order`

```sql
CREATE MATERIALIZED VIEW mart.mv_sales_order AS
SELECT
    s.*,
    c.customer_type,
    c.loyalty_tier,
...
CREATE INDEX mv_sales_order_date_branch_idx
    ON mart.mv_sales_order (business_event_date, branch_code);
CREATE INDEX mv_sales_order_customer_idx
    ON mart.mv_sales_order (customer_id);
CREATE INDEX mv_sales_order_payment_method_idx
    ON mart.mv_sales_order (payment_method);
CREATE INDEX mv_sales_order_channel_idx
    ON mart.mv_sales_order (channel);
```

### 2.4 `mart.mv_sales_line`

```sql
CREATE MATERIALIZED VIEW mart.mv_sales_line AS
SELECT
    li.*,
    s.branch_code,
    s.customer_id,
...
CREATE INDEX mv_sales_line_txn_idx
    ON mart.mv_sales_line (txn_id);
CREATE INDEX mv_sales_line_sku_idx
    ON mart.mv_sales_line (sku_id);
CREATE INDEX mv_sales_line_branch_date_idx
    ON mart.mv_sales_line (branch_code, business_event_date);
CREATE INDEX mv_sales_line_category_idx
    ON mart.mv_sales_line (category, subcategory);
```

### 2.5 `mart.mv_bank_reconciliation`

```sql
CREATE MATERIALIZED VIEW mart.mv_bank_reconciliation AS
SELECT
    bt.bank_txn_id,
...
LEFT JOIN mart.mv_sales_deposit_batch_reconciliation db
...
CREATE INDEX mv_bank_reconciliation_date_account_idx
    ON mart.mv_bank_reconciliation (business_event_date, account_id);
CREATE INDEX mv_bank_reconciliation_entity_idx
    ON mart.mv_bank_reconciliation (related_entity_table, related_entity_id);
```

### 2.6 `mart.mv_vendor_payment`

```sql
CREATE MATERIALIZED VIEW mart.mv_vendor_payment AS
SELECT
    vp.*,
    v.name_en AS vendor_name_en,
...
CREATE INDEX mv_vendor_payment_vendor_date_idx
    ON mart.mv_vendor_payment (vendor_id, business_event_date);
```

### 2.7 Refresh function — call this after every ingestion run

```sql
CREATE OR REPLACE FUNCTION mart.refresh_all_materialized_views()
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    -- Order matters: mv_sales_deposit_batch_reconciliation must come before mv_bank_reconciliation
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_sales_deposit_batch_reconciliation;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_sales_order;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_sales_line;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_bank_reconciliation;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_vendor_payment;
END;
$$;
```
...
> Example: `CREATE UNIQUE INDEX mv_sales_order_txn_id_uidx ON mart.mv_sales_order (txn_id);`
...
Keep `rag.v_public_retrievable_chunks` for inspection and non-embedded fallback paths. Redefine `rag.match_public_chunks(...)` to use `rag.mv_public_retrievable_chunks` for vector retrieval.
...
FROM rag.mv_public_retrievable_chunks
...
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY rag.mv_public_retrievable_chunks;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_sales_deposit_batch_reconciliation;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_sales_order;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_sales_line;
    REFRESH MATERIALIZED VIEW CONCURRENTLY mart.mv_bank_reconciliation;
...
| `mart.v_bank_reconciliation` | 3–10s (nested CTE every call) | <50ms (materialized) |
| `mart.v_sales_order` per query | ~500ms (6-table JOIN) | <5ms (materialized) |
