...
-- so first refresh should use mart.refresh_all_materialized_views(false).
...
DROP MATERIALIZED VIEW IF EXISTS mart.mv_bank_reconciliation;
DROP MATERIALIZED VIEW IF EXISTS mart.mv_vendor_payment;
DROP MATERIALIZED VIEW IF EXISTS mart.mv_sales_line;
DROP MATERIALIZED VIEW IF EXISTS mart.mv_sales_order;
DROP MATERIALIZED VIEW IF EXISTS mart.mv_sales_deposit_batch_reconciliation;
...
CREATE INDEX mv_sales_deposit_batch_reconciliation_date_branch_idx
    ON mart.mv_sales_deposit_batch_reconciliation (business_event_date, branch_code, payment_method);
CREATE INDEX mv_sales_deposit_batch_reconciliation_status_idx
    ON mart.mv_sales_deposit_batch_reconciliation (reconciliation_status);
...
CREATE INDEX mv_sales_order_customer_date_idx
    ON mart.mv_sales_order (customer_id, business_event_date);
CREATE INDEX mv_sales_order_payment_status_date_idx
    ON mart.mv_sales_order (payment_status, business_event_date);
CREATE INDEX mv_sales_order_b2b_open_ar_idx
    ON mart.mv_sales_order (business_event_date, customer_id, net_total_thb DESC)
...
CREATE UNIQUE INDEX mv_sales_line_line_item_id_uidx
    ON mart.mv_sales_line (line_item_id);
CREATE INDEX mv_sales_line_txn_sku_idx
    ON mart.mv_sales_line (txn_id, sku_id);
CREATE INDEX mv_sales_line_sku_date_idx
    ON mart.mv_sales_line (sku_id, business_event_date);
...
CREATE INDEX mv_sales_line_category_date_idx
    ON mart.mv_sales_line (category, subcategory, business_event_date);
...
CREATE UNIQUE INDEX mv_bank_reconciliation_bank_txn_id_uidx
    ON mart.mv_bank_reconciliation (bank_txn_id);
...
CREATE INDEX mv_bank_reconciliation_amount_idx
    ON mart.mv_bank_reconciliation (amount_thb DESC);
...
CREATE UNIQUE INDEX mv_vendor_payment_payment_id_uidx
    ON mart.mv_vendor_payment (payment_id);
...
CREATE INDEX mv_vendor_payment_invoice_idx
    ON mart.mv_vendor_payment (vendor_invoice_id);
CREATE INDEX mv_vendor_payment_contract_idx
    ON mart.mv_vendor_payment (vendor_contract_version_id);
...
SELECT * FROM mart.mv_sales_deposit_batch_reconciliation;

CREATE OR REPLACE VIEW mart.v_sales_order AS
SELECT * FROM mart.mv_sales_order;

CREATE OR REPLACE VIEW mart.v_sales_line AS
SELECT * FROM mart.mv_sales_line;

CREATE OR REPLACE VIEW mart.v_bank_reconciliation AS
SELECT * FROM mart.mv_bank_reconciliation;

CREATE OR REPLACE VIEW mart.v_vendor_payment AS
SELECT * FROM mart.mv_vendor_payment;

COMMENT ON VIEW mart.v_sales_deposit_batch_reconciliation IS
    'Compatibility view over mart.mv_sales_deposit_batch_reconciliation.';
COMMENT ON VIEW mart.v_sales_order IS
    'Compatibility view over mart.mv_sales_order.';
COMMENT ON VIEW mart.v_sales_line IS
    'Compatibility view over mart.mv_sales_line.';
COMMENT ON VIEW mart.v_bank_reconciliation IS
    'Compatibility view over mart.mv_bank_reconciliation.';
COMMENT ON VIEW mart.v_vendor_payment IS
    'Compatibility view over mart.mv_vendor_payment.';
